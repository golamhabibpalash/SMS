...
DECLARE @NewAcademicClassId INT = 2;
DECLARE @OldAcademicClassId INT = 1;
DECLARE @OldAcademicSessionId INT = 4;
DECLARE @NewAcademicSessionId INT = 5;
DECLARE @NewRollPrefix VARCHAR(10)= '2507';
DECLARE @AcademicExamGroupId INT = 32;

update s
set ClassRoll = 0
from student s
where s.AcademicClassId = @NewAcademicClassId and s.AcademicSessionId = @NewAcademicSessionId;
...
SELECT @LastClassRoll = MAX(s.ClassRoll) from Student s 
where s.AcademicClassId = @NewAcademicClassId and s.AcademicSessionId = @NewAcademicSessionId;
...
UPDATE s
SET s.ClassRoll = CAST(@LastClassRoll + RowNum AS VARCHAR)
FROM Student AS s
JOIN OrderedStudents AS o ON s.Id = o.Id;

select t.UniqueId, t.ClassRoll, t.* from student t where t.AcademicClassId = 2 and t.AcademicSessionId = 5 order by t.ClassRoll ;